...
export default function StoriesSection() {
  ...
  useEffect(() => {
    fetch('/api/stories')
      ...
      .then(data => setStories((data?.items || []).slice(0, 6)))
      .catch(() => setError(true))
        ...
        {loading ? (
          <>
            ...
            <div className="hidden sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {[1, 2, 3, 4, 5, 6].map(i => (
                <div key={i} className="bg-gray-50 rounded-xl p-3.5 animate-pulse">
                  <div className="h-2.5 bg-gray-200 rounded w-1/3 mb-2.5" />
                  <div className="h-3.5 bg-gray-200 rounded w-full mb-2" />
                  <div className="h-3.5 bg-gray-200 rounded w-3/4" />
                </div>
...
function DesktopStoryCard({ story }) {
  ...
  return (
    ...
    >
      ...
      <h3 className="text-[12px] font-bold text-gray-900 leading-snug mb-1.5
